    ...
    <SidebarContext value={contextValue}>
      <div
        className={twMerge(
          ...
          "[--accent:color-mix(in_oklab,var(--color-sidebar)_95%,black_5%)]",
          "dark:[--accent:color-mix(in_oklab,var(--color-sidebar)_90%,white_10%)]",
...
const header = tv({
  ...
  variants: {
    collapsed: {
      ...
      true: "mt-2 p-5 group-data-[sidebar-intent=float]/sidebar-container:mt-2 md:mx-auto md:size-9 md:items-center md:justify-center md:rounded-lg md:p-0 md:hover:bg-(--accent)",
    },
  },
});
...
const footer = tv({
  base: [
    ...
    "**:data-[slot=menu-trigger]:hover:bg-(--accent) **:data-[slot=menu-trigger]:hover:text-fg",
  ],
  variants: {
    expanded: {
      true: "",
...
const sidebarItemStyles = tv({
  base: [
    "group/sidebar-item relative col-span-full cursor-pointer overflow-hidden rounded-lg px-[calc(var(--spacing)*2.3)] py-[calc(var(--spacing)*1.3)] text-sidebar-fg focus-visible:outline-hidden sm:text-sm/6",
